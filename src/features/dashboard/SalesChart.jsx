...
function SalesChart({ bookings, numDays }) {
  ...
  const data = allDays.map((day) => {
    return {
      ...
      totalSales: bookings.reduce((total, current) => {
        if (isSameDay(day, new Date(current.created_at))) {
          return total + current.totalPrice;
        }
        return total;
      }, 0),
      extrasSales: bookings.reduce((total, current) => {
        if (isSameDay(day, new Date(current.created_at))) {
          return total + current.extrasPrice;
        }
        return total;
      }, 0),
    };
